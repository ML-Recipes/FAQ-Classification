...
from shared.utils import dump_txt
from cm import CM
import numpy as np
import torch
...
class BertFinetuning(object):
    ...
    """
    def __init__(self, lang_code="en", pre_trained_name='bert-base-uncased', sample=False, epochs=4, batch_size=32, test_size=0.15, 
                 random_state=42, lr=2e-5, eps=1e-8, text_col="text", label_col="label"):
    ...
    def create_model(self, df, max_length, output_path):
        ...
        # define output path
        models_path = output_path + "/models/" + self.lang_code
        eval_path = output_path + "/evaluation/" + self.lang_code
        labels_path = output_path + "/labels/" + self.lang_code
        ...
        cm = CM(
            sample=self.sample, epochs=self.epochs, batch_size=self.batch_size, random_state=self.random_state, 
        ...
        dump_json(cm.dataset.label_index, labels_path + "/labels.json", sort_keys=False)

        # generate and save classification report
        report = classification_report(cm.dataset.test_y, preds, target_names=cm.dataset.class_names)
        dump_txt(report, eval_path + "/classification_report.txt")
